...
public:
    ...
    void CalcStep()
    {
        std::vector<double> tmp, partialVals(m_values.size()), previousVals = m_values ;
...
class HomeWork_Task_2 : public Runge4Solver
{
    ...
    {
        std::vector<double> ret(m_values.size());
        ret[0] = 9*m_values[0] + 24*m_values[1] + 5*cos(m_time) - (1/3)*sin(m_time);
        ret[1] = -24*m_values[0] - 51*m_values[1] - 9*cos(m_time) + (1/3)*sin(m_time);
        return ret;
    }
public:
    void analytical_Solution (double t, double &u1, double &u2)
    {
        u1 = 2*exp(-3*t) - exp(-39*t) + (1/3)*cos(t);
        u2 = -exp(-3*t) + 2*exp(-39*t) - (1/3)*cos(t);
    }
};
